...
export default class ForgeSvc extends BaseSvc {
  ...
  async getUser (session) {
    ...
    console.log(`ForgeSvc中的getUser()的session: ${JSON.stringify(session)}`)
    if (!session.forge) {

      //经过验证，session中并没有forge该字段，{"cookie":{"originalMaxAge":86400000,"expires":"2018-11-01T12:06:46.744Z","secure":false,"httpOnly":true,"path":"/"}}
      console.log(`ForgeSvc中的getUser()的session.forge不存在 session.forge: ${session.forge}`)
      return null
    }
    ///////////////////////////////////
    //注释：注释掉的源码
    ...
    console.log(`ForgeSvc中的getUser()的token：${token}`)
    console.log('该方法的返回值requestAsync： '+this.requestAsync({
  ...
  __refresh3LeggedToken (session, scope) {
    ...
    return new Promise((resolve, reject) => {
      ...
      request({
        ...
        //打印body
        console.log(`!!!!!!!get3LegToken的body是： ${JSON.stringify(body)}`)
        console.log(`!!!!!!!!get3LegToken的response是： ${JSON.stringify(response)}`)
        try {
    ...
    return new Promise( function(resolve, reject) {
      ...
      request({
        ...
        try {
          ...
          console.log(`Ajax请求，ForgeSvc中的requestAsync()的body: ${body}`)
