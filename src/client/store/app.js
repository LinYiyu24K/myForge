...
export function setUser (user) {//实际上它只能改变 用户的信息，也就是处理用户的登陆状态而已
...
const createInitialState = () => {
  ...
  const initialState = Object.assign({},
    defaultState, {
      storage: getStorage()
    })//这...为什么不直接在上面defaultState加上 storage: getStorage()  ???
  // 可能是为了和 后面databasechanged的时候的操作一致，这样assignd的话，后面修改过的storage就会直接覆盖前面的
...
export default function reducer (state = createInitialState(), action) {
  ...
  const handler = ACTION_HANDLERS[action.type]//应该是函数的handler
