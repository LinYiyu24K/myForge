...
class CoreLayout extends React.Component {
  ...
  componentWillMount () {
    ...
    this.forgeSvc.getUser().then((user) => {
      ...
      //验证：一打开网页若需要鉴权，则先执行getUser()，这是成功之后会执行（正实）
      console.log(`>>>>>>>>>>>>>>>>>>>>>>>>>>>client/layouts/CoreLayout.js的forgeSvc.getUser()得到执行并成功`)
      this.props.setUser(user)

    }, () => {

      //验证：上面的getUser()失败之后会执行以下语句（证实）
      console.log(`出错执行>>>>>>>>>>>>>>>>>>>>>>>>>>>client/layouts/CoreLayout.js的forgeSvc.getUser()得到执行并失败`)
