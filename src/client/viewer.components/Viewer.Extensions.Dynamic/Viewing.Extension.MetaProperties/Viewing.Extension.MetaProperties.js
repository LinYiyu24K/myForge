...
class MetaPropertiesExtension extends MultiModelExtensionBase {
    ...
    this.react.setState({
      properties: [],
      search: false,//search是用来控制搜索栏的开关的，如果设置成false，在界面中点击一下那个搜索的图标即可开启
      model: null,
      dbId: null
  ...
  async setModel (model) {
    ...
    await this.react.setState({//设置state里面的api
      api: this.api,
      model
    })
  ...
  //传进来的properties是一个数组
  //每个元素是一个对象，例子：{displayName: "Description", displayValue: "SUPER CAR DRIVER SEAT 1000", displayCategory: "Design Tracking Properties", attributeName: "Description", type: 20, …}
  ...
  //render的内容是renderContent()函数的内容，可以直接跳去看renderContent()函数
  render (opts) {
